...
const SettingsPage = ({settings, setSettings}) => {
    ...
    let settingsNavListMap = settingsNavList.map(nb => <NavBlock navBlockName={nb.to=='settings=security'?<>{nb.name} <img src={'/imgs/icons/settings_alert.svg'}/></>:nb.name} onClickFunc={()=>setSettings(nb.to)} link={false} to={nb.to} activeCondition={settings} key={nb.id}/>)
        ...
        <div className={css.settings_popup+` ${settings?'':css.settings_popup_hidden}`}>
            <div className={css.settings_header}>
                <h2 className={css.settings_h}>
                    <img src={'/imgs/icons/big-settings.svg'} alt="" />
                    Настройки лаунчера и игры
                </h2>
                <button onClick={()=>{setSettings('')}} className={css.settings_esc}><img src={'/imgs/icons/header/h_close.svg'} alt="" /></button>
            </div>
